(function() {
	...
	var fields = document.querySelectorAll('.Inputfield');
	...
	function evaluateOperatorWithValues(operator, v0, v1) {
		let output = false;
		v1.split('|').forEach(v => {
			if (evaluateOperatorWithSingleValue(operator, v0, v)) {
				output = true;
			}
		});
		return output;
	...
	function evaluateOperatorWithSingleValue(operator, v0, v1) {
		switch (operator) {
		case '=': // is equal to
			return v0 == v1;
	...
	function applyConditions(field) {
		...
		evaluateDependents(field.wrapper.id);
	}
	...
	function evaluateDependents(fieldId) {
	...
	fields.forEach((wrapper) => {
		...
		struct[wrapper.id] = {
			...
			showIf: wrapper.getAttribute('data-show-if') || ''
		};
	...
	fields.forEach((field) => {
		...
		var showIf = struct[field.id].showIf;
		// example: ["key=value", "age>18"]
		...
		for (var i = 0; i < rawConditions.length; i++) {
			...
			struct['wrap_Inputfield_'+fieldname].dependents.push(field.id);
		}
		
		struct[field.id].conditions = conditions;
		
		applyConditions(struct[field.id]);
	});
}());